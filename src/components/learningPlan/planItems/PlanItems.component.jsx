...
export const PlanItems = ({ info, border, planOpenState }) => {
  ...
  return (
    <div
      className={clsx(
        styles.lp_content,
        border && styles.lp_content_border_blue,
        planOpenState && styles.learning_plan_display
      )}
    >
      {info.map((e, i) => {
        return (
          <div key={nextId()} className={styles.lp_content__item}>
            <div
              className={clsx(
                styles.lp_content__item_point,
                pointColors[setColor(i)]
              )}
            >
              <h1 className={clsx("learning_plan_counter", "txt_white")}>
                {i + 1}
              </h1>
            </div>
            <p
              className={clsx(
                "txt_gray",
                "txt_gray--thematic_scope",
                styles.lp_content__item_text
              )}
            >
              {e}
            </p>
